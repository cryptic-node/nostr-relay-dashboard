...
async fn get_events(Query(params): Query<std::collections::HashMap<String, String>>, State(pool): State<SqlitePool>) -> Json<Vec<EventPreview>> {
    ...
    let previews: Vec<EventPreview> = events.into_iter().map(|row| {
        ...
        let preview = match kind {
            ...
            3 => { // Contacts — count how many "p" tags (following)
                ...
                match t {
                    "relay" => {
                        ...
                        let _ = sqlx::query("INSERT INTO upstream_relays (url, name, enabled, preloaded) VALUES (?, ?, ?, ?)")
                            .bind(url).bind(name).bind(enabled as i64).bind(preloaded as i64).execute(&pool).await;
                        relays_restored += 1;
                    }
                    "npub" => {
                        ...
                        let _ = sqlx::query("INSERT INTO monitored_npubs (npub, label) VALUES (?, ?)")
                            .bind(npub).bind(label).execute(&pool).await;
                        npubs_restored += 1;
                    }
                    "event" => {
