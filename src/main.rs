use axum::{routing::{get, post, delete}, Router, Json, extract::{State, Query, Path}, response::IntoResponse, http::StatusCode};
...
use nostr::PublicKey;
use nostr::Event as NostrEvent; // for restore validation
...
struct EventPreview {
    ...
    kind: u16,
    kind_name: String,
    preview: String,      // NEW: human-readable version
    created_at: String,
}
...
async fn get_events(Query(params): Query<HashMap<String, String>>, State(pool): State<SqlitePool>) -> Json<Vec<EventPreview>> {
    ...
    let previews: Vec<EventPreview> = events.into_iter().map(|row| {
        ...
        let preview = match kind {
            ...
            1 => if content.len() > 200 { content.chars().take(200).collect::<String>() + "…" } else { content },
...
// NEW: Backup all events as NDJSON
...
// NEW: Restore from NDJSON (validates every line)
...
// (all your existing routes below stay exactly the same — just adding the new ones)

async fn get_relays(State(pool): State<SqlitePool>) -> Json<Vec<serde_json::Value>> { /* unchanged from last version */ 
    // ... paste your existing get_relays here if you want, but it's already perfect
...
// ... all your other functions (add_relay, get_npubs, add_npub, delete_relay, delete_npub, trigger_sync) stay 100% unchanged
...
async fn main() {
    ...
    let app = Router::new()
        ...
        .route("/api/backup", get(backup))      // ← NEW
        .route("/api/restore", post(restore))   // ← NEW
