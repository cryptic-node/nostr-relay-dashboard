...
async fn backup(State(pool): State<SqlitePool>) -> impl IntoResponse {
    ...
    // Backup relays
    let relays = sqlx::query("SELECT url, name, enabled, preloaded FROM upstream_relays")
    ...
    // Backup npubs
    let npubs = sqlx::query("SELECT npub, label FROM monitored_npubs")
        .fetch_all(&pool).await.unwrap_or_default();
    for row in npubs {
        let obj = serde_json::json!({
            "type": "npub",
    ...
    // Backup events
    let events = sqlx::query("SELECT id, pubkey, kind, content, created_at, tags, sig FROM events")
        .fetch_all(&pool).await.unwrap_or_default();
    for row in events {
